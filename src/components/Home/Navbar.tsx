...
// images
// import Logo from "../../assets/icons/shopify-logo.svg"
...
const Navbar = () => {
  ...
  const [ token, setToken ] = useState( [] ); // State to store user data
  ...
  useEffect( () => {
    // Retrieve username and pincode from localStorage
    const storedUserData = localStorage.getItem( "token" );
    if ( storedUserData ) {
      setToken( JSON.parse( storedUserData ) ); // Parse the JSON string to object
    }
  }, [] );
      ...
      <header className={ `${ active === true ? "shadow-sm fixed top-0 left-0 w-full z-10 transition-all ease-in-out " : null } w-full header-upper py-3` }>
        <div className="h-[100%] flex justify-between items-center">
          ...
          <div className="logo sm:w-1/6 w-5/12 text-center">
            <h1>
              <Link to="/" className="text-white"><img style={ { marginTop: '10px' } } src={ Logo } className="bg-transparent" width="65" alt="logo" /></Link>
            </h1>
          </div>
                    ...
                    <div className="absolute left-0 top-10 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black  focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" >
                      <div className="py-1" role="none">
                        { categories?.map( ( cat: Category ) => (
                          <button
                            type="button"
                            onClick={ () => { handleCategory( cat?.id ); setSelectedCat( cat?.title ); } }
                            key={ cat?.id }
                            className="hover:text-black hover:bg-gray-200 bg-opacity-90 pl-5 my-2 w-full text-left"
                          >
                            { cat?.title }
                          </button>
                        ) ) }
