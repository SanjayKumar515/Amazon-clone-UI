...
import { IProductState, addToWishlist, removeFromWishlist } from "../../redux/reducers/product/productSlice"
    ...
    = ({ data, isHidden, wishlist }) => {
        ...
        const ToWishlist = useCallback(
            ...
            // },
                ...
                setLoading(true);
                if (wishlist?.find((item: IProductState) => item.id === id)) {
                    ...
                    <div className="group gap-3 bg-white dark:bg-[#f5f5f5] h-[250px] flex px-2 rounded-lg border text-gray-700 shadow transition hover:shadow-lg">
                        ...
                        <div className="w-[75%] relative pr-8 text-left overflow-hidden">
                            <Link to={`/product/${data.id}`} >
                                ...
                                <div>
                                    <span className="text-[#1c1b1b] font-[1rem] font-Rubik mr-3">${data?.price}</span>
                                    <span className="line-through italic text-[#1c1b1b] font-[1rem]  font-Rubik">${data?.price + data?.price * 0.3}</span>
                                </div>
                                <p>Save extra with no cost EMI</p>
                            </Link>
                            <div className="cursor-pointer ">
                                <div onClick={() => ToWishlist(data?.id as string)} className="tooltip sm:absolute hidden sm:block sm:top-4 sm:right-2 cursor-pointer hover:bg-[#c4bfbf] rounded-full p-2 " >
                                    {isLoading ?
                                        <MoonLoader
                                            size={20}
                                            color="#361AE3"
                                            ...
                                            data-testid="loader"
                                        />
                                        : wishlist?.find((id: any) => id?.id === data?.id)
