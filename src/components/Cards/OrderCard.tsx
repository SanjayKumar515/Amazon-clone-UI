            ...
            <section onClick={ () => setOpenOrder( !openOrder ) } className='px-4 sm:cursor-pointer relative py-2 flex gap-5 items-center hover:shadow-lg '>
                ...
                <div className='space-y-1'>
                    ...
                    <div className='flex flex-wrap sm:gap-3 gap-x-2'>
                        ...
                        <time className='text-[#777777] text-[.91rem] sm:text-[1rem]'>Ordered on: { new Date( item.paidAt ).toLocaleDateString() } </time>
                    </div>
                    <div className='gap-3 flex flex-wrap'>
                        <p className='text-skin-base'>price:$<span className='font-bold'>{ order?.product?.price }</span>
                        </p>
                            ...
                            {/* <div>
                                    {
                                        order?.orderStatus !== "Cancelled" ?
                                            <button onClick={() => cancelOrder(item?.id)} className='absolute bottom-3 right-3 bg-red-500 hover:bg-red-700 hover:shadow-lg px-2 py-1 rounded-md text-white'>Cancel</button>
                                            : null
                                    }
                                </div> */}
                        </section>
