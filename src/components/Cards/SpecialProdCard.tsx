        ...
        <section className="bg-skin-background  border hover:shadow-slate-300 w-full h-[300px] p-[10px] rounded-lg shadow-lg">
            <div className="flex w-full h-full items-center gap-3">
                <div className="w-1/2 h-full relative">
                    ...
                    <Link to={ `/product/${ data?.id }` } className="flex  h-[65%]  items-center justify-center">
                        ...
                        <Slider responsive={ responsive }>
                            {
                                data?.images?.map( ( item ) => (
                                    <button
                                        key={ item.url }
                                        type="button"
                                        onClick={ () => Connection( item ) }
                                        className="border overflow-hidden focus:border-black"
                                        tabIndex={ 0 }
                ...
                <div className="w-1/2 h-full  relative">
                    <button
                        type="button"
                        onClick={ () => ToWishlist( data?.id as string ) }
                        className="bg-[#c4bfbf] p-2 rounded-full absolute tooltip cursor-pointer right-0 top-0"
                    ...
                    >
                        { ( () => {
                            ...
                            if ( wishlist?.find( ( id: any ) => id?.id === data?.id ) ) {
                                return <AiFillHeart className="text-red-500" size={ 25 } />;
                            }
